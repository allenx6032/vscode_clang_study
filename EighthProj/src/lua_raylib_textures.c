...
int lua_LoadTexture(lua_State *L) {
    ...
    Texture2D *pTexture = (Texture2D *)lua_newuserdata(L, sizeof(Texture2D));
    *pTexture = texture;
...
int lua_UnloadTexture(lua_State *L) {
    Texture2D *texture = (Texture2D *)luaL_checkudata(L, 1, "Texture");
...
int lua_ImageCopy(lua_State *L) {
    ...
    Image *pCopiedImage = (Image *)lua_newuserdata(L, sizeof(Image));
...
int lua_LoadTextureCubemap(lua_State *L) {
    ...
    TextureCubemap *pCubemap = (TextureCubemap *)lua_newuserdata(L, sizeof(TextureCubemap));
    *pCubemap = cubemap;
...
int lua_LoadImageFromMemory(lua_State *L) {
    const char *fileType = (const char *)luaL_checkstring(L, 1);
    const unsigned char *fileData = (unsigned char *)luaL_checkstring(L, 2);
    int dataSize = luaL_checkinteger(L, 3);
...
int lua_UnloadImagePalette(lua_State *L) {
    Color *palette = (Color *)lua_touserdata(L, 1);
    if (palette != NULL) {
...
int lua_ImageDrawTriangleFan(lua_State *L) {
    ...
    Vector2 *points = (Vector2 *)malloc(count * sizeof(Vector2));
...
int lua_LoadRenderTexture(lua_State *L) {
    ...
    RenderTexture2D *pRenderTexture = ( RenderTexture2D *)lua_newuserdata(L, sizeof(RenderTexture2D));
...
int lua_IsRenderTextureValid(lua_State *L) {
    RenderTexture2D *renderTexture = (RenderTexture2D *)luaL_checkudata(L, 1, "RenderTexture2D");
