...
int lua_LoadModel(lua_State *L) {
    ...
    Model *pModel = (Model *)lua_newuserdata(L, sizeof(Model));
    *pModel = model;
...
int lua_UpdateModelAnimation(lua_State *L) {
    Model *model = (Model *)luaL_checkudata(L, 1, "Model");
    ModelAnimation *animation = (ModelAnimation *)luaL_checkudata(L, 2, "ModelAnimation");
...
int lua_GenMeshCube(lua_State *L) {
    ...
    Mesh *pCube = (Mesh *)lua_newuserdata(L, sizeof(Mesh));
    *pCube = cube;
...
int lua_GenMeshSphere(lua_State *L) {
    ...
    Mesh *pSphere = (Mesh *)lua_newuserdata(L, sizeof(Mesh));
    *pSphere = sphere;
    luaL_setmetatable(L, "Mesh");
    return 1;
}
...
int lua_GenMeshPlane(lua_State *L) {
    ...
    Mesh *pPlane = (Mesh *)lua_newuserdata(L, sizeof(Mesh));
    *pPlane = plane;
    luaL_setmetatable(L, "Mesh");
    return 1;
}
...
int lua_GenMeshPoly(lua_State *L) {
    ...
    Mesh *pMesh = (Mesh *)lua_newuserdata(L, sizeof(Mesh));
    *pMesh = mesh;
    luaL_setmetatable(L, "Mesh");
    return 1;
}
...
int lua_LoadMaterials(lua_State *L) {
    ...
    for (int i = 0; i < materialCount; i++) {
        Material *pMaterial = (Material *)lua_newuserdata(L, sizeof(Material));
...
int lua_LoadModelAnimations(lua_State *L) {
    ...
    for (int i = 0; i < animCount; i++) {
        ModelAnimation *pAnim = (ModelAnimation *)lua_newuserdata(L, sizeof(ModelAnimation));
        *pAnim = animations[i];
...
int lua_UnloadModelAnimation(lua_State *L) {
    ModelAnimation *animation = (ModelAnimation *)luaL_checkudata(L, 1, "ModelAnimation");
