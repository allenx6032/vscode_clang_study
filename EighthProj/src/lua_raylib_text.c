...
int lua_GetFontDefault(lua_State *L) {
    ...
    Font *pFont = (Font *)lua_newuserdata(L, sizeof(Font));
    *pFont = font;
...
int lua_GetGlyphInfo(lua_State *L) {
    ...
    GlyphInfo *pGlyphInfo = (GlyphInfo *)lua_newuserdata(L, sizeof(GlyphInfo));
...
int lua_TextJoin(lua_State *L) {
    ...
    const char **textList = (const char **)malloc(sizeof(char *) * count);
...
int lua_TextAppend(lua_State *L) {
    ...
    char *buffer = (char *)malloc(textLen + appendLen + 1);
    if (!buffer) {
