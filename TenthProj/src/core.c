...
static void setBufferData( lua_State* L, Buffer* buffer, int index, size_t offset ) {
	int t = index;
	unsigned char* ucp = (unsigned char* )(buffer->data) + offset;
	unsigned short* usp = (unsigned short* )(buffer->data) + offset;
	unsigned int* uip = (unsigned int* )(buffer->data) + offset;
	unsigned long* ulp = (unsigned long* )(buffer->data) + offset;
	char* cp = (char* )(buffer->data) + offset;
	short* sp = (short* )(buffer->data) + offset;
	int* ip = (int* )(buffer->data) + offset;
	long* lp = (long* )(buffer->data)+ offset;
	float* fp = (float* )(buffer->data) + offset;
	double* dp = (double* )(buffer->data) + offset;
...
int lcoreExportDataAsCode( lua_State* L ) {
	...
	lua_pushboolean( L, ExportDataAsCode( (const unsigned char *)buffer->data, buffer->size, fileName ) );
...
int lcoreCompressData( lua_State* L ) {
	...
	unsigned char* compData = CompressData( (const unsigned char *)inBuffer->data, inBuffer->size, (int*)&outBuffer.size );
...
int lcoreDecompressData( lua_State* L ) {
	...
	unsigned char* data = DecompressData( (const unsigned char *)inBuffer->data, inBuffer->size, (int*)&outBuffer.size );
...
int lcoreEncodeDataBase64( lua_State* L ) {
	int dataSize = 0;
	const unsigned char* string = (const unsigned char*)luaL_checklstring( L, 1, (size_t*)&dataSize );

	int outputSize = 0;
	const char* compData = EncodeDataBase64( string, dataSize, &outputSize );
...
int lcoreDecodeDataBase64( lua_State* L ) {
	int outputSize = 0;
	unsigned char* decodedData = DecodeDataBase64( (const unsigned char *)luaL_checkstring( L, 1 ), &outputSize );

	lua_pushstring( L, (const char *)decodedData );
	lua_pushinteger( L, outputSize );
...
int lcoreComputeCRC32( lua_State* L ) {
	...
	if ( buffer->type == BUFFER_UNSIGNED_CHAR ) {
		lua_pushinteger( L, ComputeCRC32( (unsigned char *)buffer->data, buffer->size ) );
	}
	else {
...
int lcoreComputeMD5( lua_State* L ) {
	...
	if ( buffer->type == BUFFER_UNSIGNED_CHAR ) {
		unsigned int* code = ComputeMD5( (unsigned char *)buffer->data, buffer->size );
...
int lcoreComputeSHA1( lua_State* L ) {
	...
	if ( buffer->type == BUFFER_UNSIGNED_CHAR ) {
		unsigned int* code = ComputeSHA1( (unsigned char *)buffer->data, buffer->size );
...
int lcoreCopyBufferData( lua_State* L ) {
	...
	void* dstP = (Buffer*)dst->data + posDst * getBufferElementSize( dst );
	void* srcP = (Buffer*)src->data + posSrc * getBufferElementSize( src );
...
int lcoreSwapBufferEndianness( lua_State* L ) {
	...
	if ( buffer->type == BUFFER_UNSIGNED_SHORT ) {
		unsigned short *p = (unsigned short *)buffer->data;
		...
		unsigned int *p = (unsigned int *)buffer->data;
		...
		unsigned long *p = (unsigned long *)buffer->data;
...
int lcoreGetBufferData( lua_State* L ) {
	...
	if ( buffer->type == BUFFER_UNSIGNED_CHAR ) {
		unsigned char* p = (unsigned char* )buffer->data + position * sizeof( unsigned char );
		lua_createtable( L, count, 0 );
		...
		unsigned short *p = (unsigned short *)buffer->data + position * sizeof( unsigned short );
		lua_createtable( L, count, 0 );
		...
		unsigned int* p = (unsigned int*)buffer->data + position * sizeof( unsigned int );
		lua_createtable( L, count, 0 );
		...
		unsigned long* p = (unsigned long*)buffer->data + position * sizeof( unsigned long );
		lua_createtable( L, count, 0 );
		...
		char* p = (char* )buffer->data + position * sizeof( char );
		lua_createtable( L, count, 0 );
		...
		short *p = (short *)buffer->data + position * sizeof( short );
		lua_createtable( L, count, 0 );
		...
		int* p = (int*)buffer->data + position * sizeof( int );
		lua_createtable( L, count, 0 );
		...
		long* p = (long*)buffer->data + position * sizeof( long );
		lua_createtable( L, count, 0 );
		...
		float* p = (float*)buffer->data + position * sizeof( float );
		lua_createtable( L, count, 0 );
		...
		double* p = (double*)buffer->data + position * sizeof( double );
		lua_createtable( L, count, 0 );
