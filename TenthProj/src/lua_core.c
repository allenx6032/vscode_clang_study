...
// #ifdef PLATFORM_DESKTOP
	#include "core_desktop_glfw.c.h"
// #elif PLATFORM_DESKTOP_SDL2
// 	#include "core_desktop_sdl2.c.h"
// #elif PLATFORM_DESKTOP_SDL3
// 	#include "core_desktop_sdl3.c.h"
// #elif PLATFORM_WEB
// 	#include "core_web.c.h"
// #endif
...
static int gcBuffer( lua_State* L ) {
	if ( state->gcUnload ) {
		Buffer* buffer = (Buffer*)luaL_checkudata( L, 1, "Buffer" );
...
static int gcRenderTexture( lua_State* L ) {
	if ( state->gcUnload ) {
		RenderTexture* renderTexture = (RenderTexture*)luaL_checkudata( L, 1, "RenderTexture" );
...
static int gcGlyphInfo( lua_State* L ) {
	if ( state->gcUnload ) {
		GlyphInfo* glyph = (GlyphInfo* )luaL_checkudata( L, 1, "GlyphInfo" );
...
static int gcSoundAlias( lua_State* L ) {
	if ( state->gcUnload ) {
		Sound* sound = (Sound*)luaL_checkudata( L, 1, "SoundAlias" );
...
static int gcModelAnimation( lua_State* L ) {
	if ( state->gcUnload ) {
		ModelAnimation* modelAnimation = (ModelAnimation*)luaL_checkudata( L, 1, "ModelAnimation" );
...
static int gcRLRenderBatch( lua_State* L ) {
	if ( state->gcUnload ) {
		rlRenderBatch* renderBatch = (rlRenderBatch*)luaL_checkudata( L, 1, "rlRenderBatch" );
...
static int gcAutomationEventList( lua_State* L ) {
	if ( state->gcUnload ) {
		AutomationEventList* automationEventList = (AutomationEventList*)luaL_checkudata( L, 1, "AutomationEventList" );
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "buffer", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					buffer = (Buffer*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					buffer = (Buffer*)luaL_checkudata( L, lua_gettop( L ), "Buffer" );
				}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "image", lua_tostring( L, -2 ) ) ) {
				Image* image = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					image = (Image*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					image = (Image*)luaL_checkudata( L, lua_gettop( L ), "Image" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return image;
			}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "texture", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					texture = (Texture*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "renderTexture", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					renderTexture = (RenderTexture*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					renderTexture = (RenderTexture*)luaL_checkudata( L, lua_gettop( L ), "RenderTexture" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (RenderTexture*)luaL_checkudata( L, index, "RenderTexture" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "shader", lua_tostring( L, -2 ) ) ) {
				Shader* shader = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					shader = (Shader*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "mesh", lua_tostring( L, -2 ) ) ) {
				Mesh* mesh = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					mesh = (Mesh*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					mesh = (Mesh*)luaL_checkudata( L, lua_gettop( L ), "Mesh" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return mesh;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Mesh*)luaL_checkudata( L, index, "Mesh" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "camera2D", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					camera = (Camera2D*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					camera = (Camera2D*)luaL_checkudata( L, lua_gettop( L ), "Camera2D" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return camera;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Camera2D*)luaL_checkudata( L, index, "Camera2D" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "camera3D", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					camera = (Camera3D*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					camera = (Camera3D*)luaL_checkudata( L, lua_gettop( L ), "Camera3D" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return camera;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Camera3D*)luaL_checkudata( L, index, "Camera3D" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "font", lua_tostring( L, -2 ) ) ) {
				Font* font = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					font = (Font*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					font = (Font*)luaL_checkudata( L, lua_gettop( L ), "Font" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return font;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Font*)luaL_checkudata( L, index, "Font" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "glyphInfo", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					glyph = (GlyphInfo*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					glyph = (GlyphInfo*)luaL_checkudata( L, lua_gettop( L ), "GlyphInfo" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return glyph;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (GlyphInfo*)luaL_checkudata( L, index, "GlyphInfo" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "wave", lua_tostring( L, -2 ) ) ) {
				Wave* wave = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					wave = (Wave*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					wave = (Wave*)luaL_checkudata( L, lua_gettop( L ), "Wave" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return wave;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Wave*)luaL_checkudata( L, index, "Wave" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "sound", lua_tostring( L, -2 ) ) ) {
				Sound* sound = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					sound =  (Sound*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					sound = (Sound*)luaL_checkudata( L, lua_gettop( L ), "Sound" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return sound;
			}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "music", lua_tostring( L, -2 ) ) ) {
				Music* music = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					music = (Music*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					music = (Music*)luaL_checkudata( L, lua_gettop( L ), "Music" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return music;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Music*)luaL_checkudata( L, index, "Music" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "light", lua_tostring( L, -2 ) ) ) {
				Light* light = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					light = (Light*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					light = (Light*)luaL_checkudata( L, lua_gettop( L ), "Light" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return light;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Light*)luaL_checkudata( L, index, "Light" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "material", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					material = (Material*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					material = (Material*)luaL_checkudata( L, lua_gettop( L ), "Material" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return material;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (Material*)luaL_checkudata( L, index, "Material" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "model", lua_tostring( L, -2 ) ) ) {
				Model* model = NULL;
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					model =  (Model*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					model = (Model*)luaL_checkudata( L, lua_gettop( L ), "Model" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return model;
			}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "modelAnimation", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					animation = (ModelAnimation*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					animation = (ModelAnimation*)luaL_checkudata( L, lua_gettop( L ), "ModelAnimation" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return animation;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (ModelAnimation*)luaL_checkudata( L, index, "ModelAnimation" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "rlRenderBatch", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					batch = (rlRenderBatch*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					batch = (rlRenderBatch*)luaL_checkudata( L, lua_gettop( L ), "rlRenderBatch" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return batch;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (rlRenderBatch*)luaL_checkudata( L, index, "rlRenderBatch" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "automationEvent", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					event = (AutomationEvent* )lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					event = (AutomationEvent* )luaL_checkudata( L, lua_gettop( L ), "AutomationEvent" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return event;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (AutomationEvent* )luaL_checkudata( L, index, "AutomationEvent" );
	}
}
	...
	case LUA_TTABLE:
	{
		...
		while ( lua_next( L, t ) != 0 ) {
			if ( TextIsEqual( "automationEventList", lua_tostring( L, -2 ) ) ) {
				...
				if ( lua_islightuserdata( L, lua_gettop( L ) ) ) {
					list = (AutomationEventList*)lua_touserdata( L, lua_gettop( L ) );
				}
				else {
					list = (AutomationEventList*)luaL_checkudata( L, lua_gettop( L ), "AutomationEventList" );
				}
				lua_pop( L, 2 ); /* Pops also the string. */
				return list;
			}
		...
		/* Don't brake here, we want to get error from default if not found. */
	default:
		return (AutomationEventList*)luaL_checkudata( L, index, "AutomationEventList" );
	}
}
...
void uluaPushBuffer( lua_State* L, Buffer buffer ) {
	...
	Buffer* bufferP = (Buffer*)lua_newuserdata( L, sizeof( Buffer ) );
	*bufferP = buffer;
...
void uluaPushImage( lua_State* L, Image image ) {
	Image* imageP = (Image*)lua_newuserdata( L, sizeof( Image ) );
	*imageP = image;
...
void uluaPushTexture( lua_State* L, Texture texture ) {
	Texture* textureP = (Texture*)lua_newuserdata( L, sizeof( Texture ) );
	*textureP = texture;
...
void uluaPushRenderTexture( lua_State* L, RenderTexture renderTexture ) {
	RenderTexture* renderTextureP = (RenderTexture*)lua_newuserdata( L, sizeof( RenderTexture ) );
...
void uluaPushCamera2D( lua_State* L, Camera2D camera ) {
	Camera2D* cameraP = (Camera2D*)lua_newuserdata( L, sizeof( Camera2D ) );
	*cameraP = camera;
	luaL_setmetatable( L, "Camera2D" );
}

void uluaPushCamera3D( lua_State* L, Camera3D camera ) {
	Camera3D* cameraP = (Camera3D* )lua_newuserdata( L, sizeof( Camera3D ) );
	*cameraP = camera;
	luaL_setmetatable( L, "Camera3D" );
}

void uluaPushShader( lua_State* L, Shader shader ) {
	Shader* shaderP = (Shader*)lua_newuserdata( L, sizeof( Shader ) );
	*shaderP = shader;
...
void uluaPushFont( lua_State* L, Font font ) {
	Font* fontP = (Font*)lua_newuserdata( L, sizeof( Font ) );
	*fontP = font;
...
void uluaPushGlyphInfo( lua_State* L, GlyphInfo glyph ) {
	GlyphInfo* glyphP = (GlyphInfo*)lua_newuserdata( L, sizeof( GlyphInfo ) );
	*glyphP = glyph;
...
void uluaPushWave( lua_State* L, Wave wave ) {
	Wave* waveP = (Wave*)lua_newuserdata( L, sizeof( Wave ) );
	*waveP = wave;
...
void uluaPushSound( lua_State* L, Sound sound ) {
	Sound* soundP = (Sound*)lua_newuserdata( L, sizeof( Sound ) );
	*soundP = sound;
...
void uluaPushSoundAlias( lua_State* L, Sound alias ) {
	Sound* aliasP = (Sound*)lua_newuserdata( L, sizeof( Sound ) );
	*aliasP = alias;
...
void uluaPushMusic( lua_State* L, Music music ) {
	Music* musicP = (Music*)lua_newuserdata( L, sizeof( Music ) );
	*musicP = music;
...
void uluaPushLight( lua_State* L, Light light ) {
	Light* lightP = (Light*)lua_newuserdata( L, sizeof( Light ) );
	*lightP = light;
...
void uluaPushMaterial( lua_State* L, Material material ) {
	Material* materialP = (Material*)lua_newuserdata( L, sizeof( Material ) );
	*materialP = material;
...
void uluaPushMesh( lua_State* L, Mesh mesh ) {
	Mesh* meshP = (Mesh*)lua_newuserdata( L, sizeof( Mesh ) );
	*meshP = mesh;
...
void uluaPushModel( lua_State* L, Model model ) {
	Model* modelP = (Model* )lua_newuserdata( L, sizeof( Model ) );
	*modelP = model;
...
void uluaPushModelAnimation( lua_State* L, ModelAnimation modelAnimation ) {
	ModelAnimation* modelAnimationP = (ModelAnimation*)lua_newuserdata( L, sizeof( ModelAnimation ) );
...
void uluaPushRLRenderBatch( lua_State* L, rlRenderBatch renderBatch ) {
	rlRenderBatch* renderBatchP = (rlRenderBatch*)lua_newuserdata( L, sizeof( rlRenderBatch ) );
...
void uluaPushAutomationEvent( lua_State* L, AutomationEvent event ) {
	AutomationEvent* eventP = (AutomationEvent*)lua_newuserdata( L, sizeof( AutomationEvent ) );
	*eventP = event;
...
void uluaPushAutomationEventList( lua_State* L, AutomationEventList eventList ) {
	AutomationEventList* eventListP = (AutomationEventList*)lua_newuserdata( L, sizeof( AutomationEventList ) );
