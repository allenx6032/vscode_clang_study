...
int ltextLoadFontFromMemory( lua_State* L ) {
	...
	if ( lua_istable( L, 4 ) ) {
		...
		uluaPushFont( L, LoadFontFromMemory( fileType, ( const unsigned char *)fileData->data, fileData->size, fontSize, codepoints, codepointCount ) );

		return 1;
	}
	/* If no codepoints provided. */
	uluaPushFont( L, LoadFontFromMemory( fileType, ( const unsigned char *)fileData->data, fileData->size, fontSize, NULL, 0 ) );
...
int ltextLoadFontFromData( lua_State* L ) {
	...
	while ( lua_next( L, t ) != 0 ) {
		if ( strcmp( "baseSize", (char*)lua_tostring( L, -2 ) ) == 0 ) {
			...
			font.recs = (Rectangle *)malloc( recCount * sizeof( Rectangle ) );
			int t2 = lua_gettop( L );
			int i = 0;
			lua_pushnil( L );
...
int ltextLoadFontData( lua_State* L ) {
	...
	if ( lua_istable( L, 3 ) ) {
		...
		getCodepoints( L, codepoints, 3 );
		GlyphInfo* glyphs = LoadFontData( (const unsigned char *)fileData->data, fileData->size, fontSize, codepoints, codepointCount, type );
	...
	/* If no codepoints provided. */
	GlyphInfo* glyphs = LoadFontData( (const unsigned char *)fileData->data, fileData->size, fontSize, NULL, 0, type );
	lua_createtable( L, codepointCount, 0 );
