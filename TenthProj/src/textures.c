...
int ltexturesLoadImageAnimFromMemory( lua_State* L ) {
	...
	int frameCount = 0;
	uluaPushImage( L, LoadImageAnimFromMemory( fileType, (const unsigned char *)buffer->data, buffer->size, &frameCount ) );
	lua_pushinteger( L, frameCount );
...
int ltexturesLoadImageFromMemory( lua_State* L ) {
	...
	uluaPushImage( L, LoadImageFromMemory( fileType, (const unsigned char *)data->data, data->size ) );
