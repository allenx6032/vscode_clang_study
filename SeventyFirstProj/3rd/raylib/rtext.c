...
extern void LoadFontDefault(void)
{
    ...
    defaultFont.recs = (RectangleR *)RL_CALLOC(defaultFont.glyphCount, sizeof(RectangleR));
...
#if defined(SUPPORT_FILEFORMAT_TTF) || defined(SUPPORT_FILEFORMAT_BDF)
Image GenImageFontAtlas(const GlyphInfo *glyphs, RectangleR **glyphRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    Image atlas = { 0 };
...
static Font LoadBMFont(const char *fileName)
{
    ...
    if (pageCount > 1)
    {
        ...
        {
            RectangleR srcRec = { 0.0f, 0.0f, (float)imWidth, (float)imHeight };
            RectangleR destRec = { 0.0f, (float)imHeight*(float)i, (float)imWidth, (float)imHeight };
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        {
            // Get character rectangle in the font atlas texture
            font.recs[i] = (RectangleR){ (float)charX, (float)charY + (float)imHeight*pageID, (float)charWidth, (float)charHeight };
