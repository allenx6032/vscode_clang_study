...
Image ImageFromImage(Image image, RectangleR rec)
{
    Image result = { 0 };
...
void ImageCrop(Image *image, RectangleR crop)
{
    ...
    {
        TRACELOG(LOG_WARNING, "IMAGE: Failed to crop, RectangleR out of bounds");
        return;
    }
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        else
        {
            ...
            {
                RectangleR rec = { (float)(textOffsetX + font.glyphs[index].offsetX), (float)(textOffsetY + font.glyphs[index].offsetY), (float)font.recs[index].width, (float)font.recs[index].height };
                ImageDraw(&imText, font.glyphs[index].image, (RectangleR){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
...
void ImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill)
{
    ...
    {
        RectangleR srcRec = { 0, 0, (float)image->width, (float)image->height };
...
void ImageAlphaCrop(Image *image, float threshold)
{
    ...
    RectangleR crop = GetImageAlphaBorder(*image, threshold);
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
RectangleR GetImageAlphaBorder(Image image, float threshold)
{
    RectangleR crop = { 0 };
    ...
    if (pixels != NULL)
    {
        ...
        {
            crop = (RectangleR){ (float)xMin, (float)yMin, (float)((xMax + 1) - xMin), (float)((yMax + 1) - yMin) };
        }
...
{
    ImageDrawRectangleRec(dst, (RectangleR){ (float)posX, (float)posY, (float)width, (float)height }, color);
}
...
// Draw rectangle within an image
void ImageDrawRectangleRec(Image *dst, RectangleR rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
...
void ImageDrawRectangleLines(Image *dst, RectangleR rec, int thick, Color color)
{
...
void ImageDraw(Image *dst, Image src, RectangleR srcRec, RectangleR dstRec, Color tint)
{
    // Security check to avoid program crash
    ...
    else
    {
        ...
        if (((int)srcRec.width != (int)dstRec.width) || ((int)srcRec.height != (int)dstRec.height))
        {
            ...
            srcRec = (RectangleR){ 0, 0, (float)srcMod.width, (float)srcMod.height };
        ...
        if ((dst->mipmaps > 1) && (src.mipmaps > 1))
        {
            ...
            RectangleR mipmapSrcRec = srcRec;
            mipmapSrcRec.width /= 2;
            mipmapSrcRec.height /= 2;
            mipmapSrcRec.x /= 2;
            mipmapSrcRec.y /= 2;

            RectangleR mipmapDstRec = dstRec;
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    ...
    RectangleR srcRec = { 0.0f, 0.0f, (float)imText.width, (float)imText.height };
    RectangleR dstRec = { position.x, position.y, (float)imText.width, (float)imText.height };
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        RectangleR faceRecs[6] = { 0 };      // Face source rectangles

        for (int i = 0; i < 6; i++) faceRecs[i] = (RectangleR){ 0, 0, (float)size, (float)size };
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, mipmapped, faceRecs[i], (RectangleR){ 0, (float)size*i, (float)size, (float)size }, WHITE);
...
// NOTE: pixels data must match texture.format
void UpdateTextureRec(Texture2D texture, RectangleR rec, const void *pixels)
{
...
{
    RectangleR source = { 0.0f, 0.0f, (float)texture.width, (float)texture.height };
    RectangleR dest = { position.x, position.y, (float)texture.width*scale, (float)texture.height*scale };
...
void DrawTextureRec(Texture2D texture, RectangleR source, Vector2 position, Color tint)
{
    RectangleR dest = { position.x, position.y, fabsf(source.width), fabsf(source.height) };
    Vector2 origin = { 0.0f, 0.0f };
...
void DrawTexturePro(Texture2D texture, RectangleR source, RectangleR dest, Vector2 origin, float rotation, Color tint)
{
...
void DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, RectangleR dest, Vector2 origin, float rotation, Color tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
