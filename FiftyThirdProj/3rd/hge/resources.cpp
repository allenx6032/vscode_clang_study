...
uint32_t RResource::Get(hgeResourceManager *rm) {
  if (!handle) {
    handle = static_cast<int32_t>(reinterpret_cast<uintptr_t>(hge_->Resource_Load(filename.c_str())));
  }
  return handle;
}
...
uint32_t RSprite::Get(hgeResourceManager *rm) {
  if (!handle) {
    ...
    handle = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(spr));
  }
  return handle;
}
...
uint32_t RFont::Get(hgeResourceManager *rm) {
  if (!handle) {
    ...
    handle = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(fnt));
  }
  return handle;
}
...
uint32_t RParticle::Get(hgeResourceManager *rm) {
  if (!handle) {
    ...
    handle = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(par));
  }
  return handle;
}
...
uint32_t RDistort::Get(hgeResourceManager *rm) {
  if (!handle) {
    ...
    handle = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(dis));
  }
  return handle;
}
...
uint32_t RStringTable::Get(hgeResourceManager *rm) {
  if (!handle) {
    handle = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(new hgeStringTable(filename.c_str())));
  }
  return handle;
}
