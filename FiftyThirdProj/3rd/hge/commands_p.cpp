...
void cmdSavePreset(const int n) {
  ...
  GetModuleFileName(GetModuleHandle(nullptr), utf8_to_wchar(filename), _MAX_PATH);
  string s(filename);
  ...
  const auto h_f = CreateFile(utf8_to_wchar(filename), GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS,
...
void cmdLoadPreset(const int n) {
  ...
  const auto h_f = CreateFile(utf8_to_wchar(filename), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                              FILE_ATTRIBUTE_NORMAL, nullptr);
  if (h_f == INVALID_HANDLE_VALUE) {
    return;
  }
  ...
  CloseHandle(h_f);
  sprParticles->SetFrame(static_cast<uint32_t>(reinterpret_cast<uintptr_t>(state.ps->info.sprite) & 0xFFFF));
  sprParticles->SetBlendMode((hgeBlendMode)(static_cast<uint32_t>(reinterpret_cast<uintptr_t>(state.ps->info.sprite) >> 16)));
  state.ps->info.sprite = sprParticles;
