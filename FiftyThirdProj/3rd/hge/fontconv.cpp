...
int main(const int argc, char *argv[]) {
  ...
  const auto h_search = FindFirstFile(utf8_to_wchar(argv[1]), &search_data);
...
bool convert(char *filename) {
  ...
  if (!DeleteFile(utf8_to_wchar(filename))) {
    printf("Can't replace file\n");
    return false;
  }
  if (!MoveFile(utf8_to_wchar(tempfile), utf8_to_wchar(filename))) {
