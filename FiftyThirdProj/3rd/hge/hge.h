...
#include <hge/impl/hgedefs.h>
#include <hge/impl/hgehandle.h>
#include <hge/impl/hgekeys.h>
#include <hge/impl/hgecolor.h>
...
static std::vector<uint32_t> utf8_to_unicode(const std::vector<uint8_t>& utf8) {
    std::vector<uint32_t> codepoints;
    size_t i = 0;
    while (i < utf8.size()) {
        uint32_t codepoint = 0;
        if ((utf8[i] & 0x80) == 0x00) {          // 1字节（0xxxxxxx）
            codepoint = utf8[i];
            i += 1;
        } else if ((utf8[i] & 0xE0) == 0xC0) {   // 2字节（110xxxxx）
            codepoint = (utf8[i] & 0x1F) << 6;
            codepoint |= (utf8[i+1] & 0x3F);
            i += 2;
        } else if ((utf8[i] & 0xF0) == 0xE0) {   // 3字节（1110xxxx）
            codepoint = (utf8[i] & 0x0F) << 12;
            codepoint |= (utf8[i+1] & 0x3F) << 6;
            codepoint |= (utf8[i+2] & 0x3F);
            i += 3;
        } else if ((utf8[i] & 0xF8) == 0xF0) {   // 4字节（11110xxx）
...
// 将 Unicode 码点列表转为 UTF-16（小端序）
static std::vector<uint16_t> unicode_to_utf16(const std::vector<uint32_t>& codepoints) {
...
// 将 UTF-16 字节流解码为 Unicode 码点列表
static std::vector<uint32_t> utf16_to_unicode(const std::vector<uint16_t>& utf16) {
    std::vector<uint32_t> codepoints;
    size_t i = 0;
    while (i < utf16.size()) {
        uint16_t word = utf16[i];
        if ((word & 0xFC00) == 0xD800) { // 高代理（0xD800 ~ 0xDBFF）
            if (i+1 >= utf16.size() || (utf16[i+1] & 0xFC00) != 0xDC00) {
                throw std::runtime_error("Invalid UTF-16 surrogate pair");
            }
            // 计算完整码点
            uint32_t high = (word & 0x3FF) << 10;
            uint32_t low = utf16[i+1] & 0x3FF;
            codepoints.push_back(0x10000 + (high | low));
            i += 2;
        } else {
            codepoints.push_back(word);
            i += 1;
        }
    }
    return codepoints;
}
// 将 Unicode 码点列表转为 UTF-8
static std::vector<uint8_t> unicode_to_utf8(const std::vector<uint32_t>& codepoints) {
    std::vector<uint8_t> utf8;
    for (uint32_t cp : codepoints) {
        if (cp <= 0x7F) {                  // 1字节
            utf8.push_back(static_cast<uint8_t>(cp));
        } else if (cp <= 0x7FF) {          // 2字节
            utf8.push_back(0xC0 | ((cp >> 6) & 0x1F));
            utf8.push_back(0x80 | (cp & 0x3F));
        } else if (cp <= 0xFFFF) {         // 3字节
            utf8.push_back(0xE0 | ((cp >> 12) & 0x0F));
            utf8.push_back(0x80 | ((cp >> 6) & 0x3F));
            utf8.push_back(0x80 | (cp & 0x3F));
        } else if (cp <= 0x10FFFF) {       // 4字节
