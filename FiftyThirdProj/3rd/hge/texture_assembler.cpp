...
void CTextureAssembler::AccumulateFileResources(char *wildcard, const int resgroup,
  ...
  const auto h_search = FindFirstFile(utf8_to_wchar(wildcard), &search_data);
  ...
  do {
    ...
    if (search_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      strcpy(temp, buf);
      strcpy(buf, wchar_to_utf8(search_data.cFileName));
      strcat(filename, "\\");
      ...
      if (CheckMask(wchar_to_utf8(search_data.cFileName), mask_set, mask_inclusive))
        if (!FindObj(obj_list, wchar_to_utf8(search_data.cFileName))) {
          strcpy(filename, wildcard);
          buf = strrchr(filename, '\\');
          if (!buf) {
            buf = filename;
          ...
          buf = new char[strlen(wchar_to_utf8(search_data.cFileName)) + 1];
          strcpy(buf, wchar_to_utf8(search_data.cFileName));
