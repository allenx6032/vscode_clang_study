...
public:
    AutoLibraryUnload(const char* moduleName) {
        #if defined(UNICODE)
            int len = MultiByteToWideChar(CP_ACP, 0, moduleName, -1, NULL, 0);
            wchar_t* moduleNameW = new wchar_t[len];
            MultiByteToWideChar(CP_ACP, 0, moduleName, -1, moduleNameW, len);
            fModule = LoadLibraryW(moduleNameW);
            delete[] moduleNameW;
        #elif
