...
namespace std {
  template<>
  struct hash<footstone::string_view::u8string> {
    size_t operator()(const footstone::string_view::u8string& s) const {
      // 将 unsigned char 转换为 char，复用 std::string 的哈希实现
...
class LogMessage {
 public:
  ...
  inline static void LogWithFormat(const char * file, int line, const char *format, ...){
    ...
    int ret = vsprintf(log_msg, format, args);
    va_end(args);
