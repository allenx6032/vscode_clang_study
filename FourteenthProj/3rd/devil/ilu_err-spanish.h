...
ILconst_string iluErrorStringsSpanish[IL_FILE_READ_ERROR - IL_INVALID_ENUM + 1] = {
	...
	IL_TEXT("formato no soportado todavia"),
...
ILconst_string iluLibErrorStringsSpanish[IL_LIB_EXR_ERROR - IL_LIB_GIF_ERROR + 1] = {
	IL_TEXT("error en la libreria gif"),  // the correct translation will be 'error en la biblioteca gif', but nobody in spain uses the word 'biblioteca' for the translation the word 'library'. Almost all Spanish programmers translate 'library' incorrectly to 'libreria'.
	IL_TEXT("error en la libreria jpeg"),
	IL_TEXT("error en la libreria png"),
	IL_TEXT("error en la libreria tiff"),
	IL_TEXT("error en la libreria mng"),
	IL_TEXT("error en la libreria jp2"),
	IL_TEXT("error en la libreria exr"),
};
