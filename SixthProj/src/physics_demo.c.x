...
int main()
{
    ...
    PhysicsBody floor = CreatePhysicsBodyRectangle((Vector2){  static_cast<float>(screenWidth/2),  static_cast<float>(screenHeight) }, 500, 100, 10);
    floor->enabled = false; // Disable body state to convert it to static (no dynamics, but collisions)

    // Create obstacle circle physics body
    PhysicsBody circle = CreatePhysicsBodyCircle((Vector2){  static_cast<float>(screenWidth/2),  static_cast<float>(screenHeight/2) }, 45, 10);
