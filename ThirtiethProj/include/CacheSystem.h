...
namespace sen {
     ...
     */
    template <typename T> std::shared_ptr<T> loadFromFile(const std::string &pathFile) = delete;

    template <> inline std::shared_ptr<sf::Font> loadFromFile<sf::Font>(const std::string &pathFile) {
    ...
    template <> inline std::shared_ptr<sf::Texture> loadFromFile<sf::Texture>(const std::string &pathFile) {
    ...
    template <> inline std::shared_ptr<sf::SoundBuffer> loadFromFile<sf::SoundBuffer>(const std::string &pathFile) {
    ...
    template <> inline std::shared_ptr<sf::Music> loadFromFile<sf::Music>(const std::string &pathFile) {
    ...
    template <typename T> using CacheMap = std::unordered_map<std::string, std::weak_ptr<T>>;
    ...
    class CacheSystem {
      ...
      private:
         ...
         */
        template <typename T> static void updateMap(CacheMap<T> &map) {
         ...
         */
        template <typename T> static CacheMap<T> &getMap() = delete;
    };

    template <> inline CacheMap<sf::Font> &CacheSystem::getMap() { return m_fonts; }

    template <> inline CacheMap<sf::Texture> &CacheSystem::getMap() { return m_textures; }

    template <> inline CacheMap<sf::SoundBuffer> &CacheSystem::getMap() { return m_soundBuffers; }

    template <> inline CacheMap<sf::Music> &CacheSystem::getMap() { return m_music; }
} // namespace sen
