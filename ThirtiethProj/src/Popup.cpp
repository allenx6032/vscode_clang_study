...
namespace sen {
    Popup::Popup(const sf::String &string, PopupBaseline baseline, float duration)
        ...
        if (baseline == PopupBaseline::BOTTOM) {
            setPosition(sf::Vector2f(Application::getInitialWindowSize().x / 2.f,
                                     Application::getInitialWindowSize().y + getSize().y / 2.f));
