...
namespace sen {
    ...
    bool Text::haveBoundsChanged() const { return m_boundsChanged; }

    void Text::setPosition(float x, float y, float speed) { sen::Transformable::setPosition(x, y, speed); }

    void Text::setPosition(float x, float y) { sf::Transformable::setPosition(x, y); }

    void Text::setPosition(const sf::Vector2f &pos) { sf::Transformable::setPosition(pos); }

    void Text::setPosition(const sf::Vector2f &pos, float speed) { sen::Transformable::setPosition(pos, speed); }

    const sf::Vector2f &Text::getPosition() const { return sf::Transformable::getPosition(); }

    void Text::move(float x, float y) { sf::Transformable::move(x, y); }

    void Text::move(const sf::Vector2f &vect) { sf::Transformable::move(vect); }

    void Text::originSet() {
        if (m_originMode == OriginMode::CENTER) {
            sf::FloatRect bounds = getLocalBounds();
            if (m_updateOnlyXAxis)
                setOrigin(sf::Vector2f(bounds.left + bounds.width / 2.f, getOrigin().y));
            else
