...
namespace sen {
    ...
    bool Button::mouseOver() { return (getGlobalBounds().contains(Application::getMappedMousePosition())); }
    void Button::addListener(ButtonEvent state, const Callback &callback, bool preventDefault) {
        if (!m_callbacks)
