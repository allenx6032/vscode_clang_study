...
namespace sen {
    Prompt::Prompt(PromptStyle style, const sf::String &message, bool pausesState)
        ...
        okButton->setOnClickCallback([this] { m_response.response = Response::TRUE; });
        auto declButton = std::make_shared<Button>("CANCEL");
        declButton->setOnClickCallback([this] { m_response.response = Response::FALSE; });
        auto latButton = std::make_shared<Button>("LATER");
        latButton->setOnClickCallback([this] { m_response.response = Response::DISCARD; });
        if (style == PromptStyle::UNARY)
            ...
            okButton->setOnClickCallback([this] {
    ...
    bool Prompt::hasResponse() const { return m_response.response != Response::NONE; }
    const Response &Prompt::getResponse() const { return m_response; }
    void Prompt::setMessage(const sf::String &message) { m_message.setString(message); }
    ...
    void Prompt::placeButtons(float gap) {
        ButtonController::placeButtons(getGlobalBounds(), gap);
