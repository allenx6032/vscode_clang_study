...
namespace sen {
    ButtonController::ButtonController(const ButtonPointerVector &buttons) : m_buttons(buttons) { placeButtons(); }
    void ButtonController::pushButtons(ButtonPointer &button) { m_buttons.push_back(button); }

    void ButtonController::removeButton() { m_buttons.pop_back(); }
    void ButtonController::update(float deltaTime) {
        ...
        if (movement != ButtonMovement::NONE) {
            ...
            if (m_buttonPlacing == ButtonPlacing::VERTICAL)
                map([jump](Button *b) { b->setPosition({b->getPosition().x, b->getPosition().y + jump}, 6.3f); });
            else
    ...
    void ButtonController::setOffset(float offset, float speed) {
        if (m_buttonPlacing == ButtonPlacing::HORIZONTAL) {
            if (m_buttons.size() > 0) {
                ...
                map([=](Button *button) { button->setPosition(button->getPosition().x, goToPos, speed); });
            }
        } else {
            if (m_buttons.size() > 0) {
                auto goToPos = m_buttons[0]->getPosition().x + offset;

                map([=](Button *button) { button->setPosition(goToPos, button->getPosition().y, speed); });
            }
        }
    }
    void ButtonController::setButtonFixedSize(const sf::Vector2f &size) {
        map([&size](Button *button) { button->setSize(size); });
    }

    void ButtonController::map(const std::function<void(Button *)> &function) {
        std::for_each(m_buttons.begin(), m_buttons.end(),
                      [&](const std::shared_ptr<Button> &ptr) { function(ptr.get()); });
    }
    sf::Vector2f ButtonController::getBiggestSizeOfButton() {
        sf::Vector2f biggestSize(0.f, 0.f);
        map([&biggestSize](Button *b) {
            if (b->getSize().x * b->getSize().y > biggestSize.x * biggestSize.y)
    ...
    void ButtonController::checkIfSameSize(float biggestSize) {
        if (m_sameSize) {
            if (m_buttonPlacing == ButtonPlacing::VERTICAL) {
                map([&biggestSize](Button *b) { b->setSize({b->getSize().x, biggestSize}); });
            } else
                map([&biggestSize](Button *b) { b->setSize({biggestSize, b->getSize().y}); });
        }
    }
} // namespace sen
