...
void ScrunchDrawDebug(struct Pile *const pile)
{
    ...
    // DrawRectangleRoundedLines(r, pack->roundness, 9, 1.0f, RED);
    DrawRectangleRoundedLines(r, pack->roundness, 9, RED);

    Vector2 dims = PileCalcFannedRect(pile, pile->fanFactor);
    r.width = dims.x;
    r.height = dims.y;
    // DrawRectangleRoundedLines(r, pack->roundness, 9, 1.0f, GREEN);
    DrawRectangleRoundedLines(r, pack->roundness, 9, GREEN);
}
