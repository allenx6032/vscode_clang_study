...
void Game::Dynamic::loadBBoxes(const std::string& filename)
{
	...
	while (file >> numberOfFrames) // any more left in the file to read?
	{
		...
		this->animData.at(std::make_pair(animname, left)).bbox.clear();
		// this->animData.at(std::pair(animname, left)).bbox.reserve(numberOfFrames);
		this->animData.at(std::make_pair(animname, left)).bbox.reserve(numberOfFrames);
		for (int i = 0; i < numberOfFrames; i++)
		{
			int l{}, t{}, w{}, h{};
			file >> l >> t >> w >> h;
			// this->animData.at(std::pair(animname, left)).bbox.emplace_back(sf::IntRect{ {l,t},{w,h} });
			this->animData.at(std::make_pair(animname, left)).bbox.emplace_back(sf::IntRect{ {l,t},{w,h} });
			// this->animData.at(std::pair(animname, left)).numFrames++;
			this->animData.at(std::make_pair(animname, left)).numFrames++;
		}
...
bool Game::Dynamic::hasBBoxesSet(const std::string animname, bool facingleft)
{
	...
	if (animData.at(std::make_pair(animname, facingleft)).bbox.size() > 0)
	{
		// if (animData.at(std::pair(animname, facingleft)).bbox.size() != animData.at(std::pair(animname, facingleft)).numFrames)
		if (animData.at(std::make_pair(animname, facingleft)).bbox.size() != animData.at(std::make_pair(animname, facingleft)).numFrames)
		{
...
void Game::Dynamic::setAABB()
{
	...
	sf::IntRect bbox = this->animData[std::make_pair(this->currentAnim, this->facingLeft)].getBBox();
...
sf::Vector2f Game::Dynamic::getCenterOffset()
{
	...
	spr.setTextureRect(my.frames.at(std::make_pair(my.currentAnim, my.facingLeft)).at(my.index));
