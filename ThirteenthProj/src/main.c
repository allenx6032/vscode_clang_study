#include "PainterEngine/PX_PainterEngine.h"

PX_Object* Previous, * Next, * Image;
px_texture my_texture[10];//存放图片的数组
px_int postion = 0;//当前图片的索引
...
PX_OBJECT_EVENT_FUNCTION(OnButtonPreClick)
{
	...
	PX_Object_ImageSetTexture(Image, &my_texture[postion]);//设置图片
}
...
PX_OBJECT_EVENT_FUNCTION(OnButtonNextClick)
{
	...
	PX_Object_ImageSetTexture(Image, &my_texture[postion]);
}
