...
extern void LoadFontDefault(void)
{
    ...
    defaultFont.recs = (RectangleR *)RL_MALLOC(defaultFont.glyphCount*sizeof(RectangleR));
...
Font LoadFontFromImage(Image image, Color key, int firstChar)
{
    ...
    RectangleR tempCharRecs[MAX_GLYPHS_FROM_IMAGE] = { 0 };
    ...
    font.recs = (RectangleR *)RL_MALLOC(font.glyphCount*sizeof(RectangleR));
...
#if defined(SUPPORT_FILEFORMAT_TTF)
Image GenImageFontAtlas(const GlyphInfo *chars, RectangleR **charRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    Image atlas = { 0 };
    ...
    *charRecs = NULL;
    ...
    RectangleR *recs = (RectangleR *)RL_MALLOC(glyphCount*sizeof(RectangleR));
...
//#define SUPPORT_FONT_DATA_COPY
    ...
    byteCount += sprintf(txtData + byteCount, "    font.recs = (RectangleR *)malloc(font.glyphCount*sizeof(RectangleR));\n");
    byteCount += sprintf(txtData + byteCount, "    memcpy(font.recs, fontRecs_%s, font.glyphCount*sizeof(RectangleR));\n\n", fileNamePascal);
...
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint)
{
    ...
    RectangleR dstRec = { position.x + font.glyphs[index].offsetX*scaleFactor - (float)font.glyphPadding*scaleFactor,
    ...
    RectangleR srcRec = { font.recs[index].x - (float)font.glyphPadding, font.recs[index].y - (float)font.glyphPadding,
...
// NOTE: If codepoint is not found in the font it fallbacks to '?'
RectangleR GetGlyphAtlasRec(Font font, int codepoint)
{
    RectangleR rec = { 0 };
...
static Font LoadBMFont(const char *fileName)
{
    ...
    font.recs = (RectangleR *)RL_MALLOC(glyphCount*sizeof(RectangleR));
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        // Get character rectangle in the font atlas texture
        font.recs[i] = (RectangleR){ (float)charX, (float)charY, (float)charWidth, (float)charHeight };
